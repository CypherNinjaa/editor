...
const IMPORT_ERROR_TIMEOUT_MS = 5000
...
function isSceneGraphLike(value: unknown): value is SceneGraphLike {
  if (!value || typeof value !== 'object') return false
  const record = value as Record<string, unknown>
  if (!record.nodes || typeof record.nodes !== 'object') return false
  if (!Array.isArray(record.rootNodeIds)) return false
  return record.rootNodeIds.every((id) => typeof id === 'string')
}
...
function LoadJsonButton({ onError }: { onError: (message: string | null) => void }) {
  ...
  const handleFileLoad = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      try {
        ...
        applySceneGraphToEditor(parsed)
        onError(null)
      } catch (error) {
        console.error('Failed to load scene JSON:', error)
        onError('Invalid scene JSON. Expected { nodes, rootNodeIds }.')
      }
    }
  ...
  return (
    <>
      <Tooltip>
        ...
        <TooltipContent side="bottom">Load JSON</TooltipContent>
      </Tooltip>
      <input
...
export function ViewerToolbarRight() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => setImportError(null), IMPORT_ERROR_TIMEOUT_MS)
